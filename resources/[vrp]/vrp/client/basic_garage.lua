  ...
  if vehicle and not IsVehicleDriveable(vehicle[3]) then -- precheck if vehicle is undriveable
	...
	if vehicle == nil then
		...
		if HasModelLoaded(mhash) then
			...
			exports['vrp_lscustoms']:setTunning(nveh, tunning)
...
AddEventHandler("gameEventTriggered", function(name, args)
    if name == "CEventNetworkPlayerEnteredVehicle" then
		...
		Citizen.CreateThread(function()
			local threadSleep = 30
			local isDriver = false
			if playerPed == PlayerPedId() then
				local vehicleId = 0
				local netId = NetworkGetNetworkIdFromEntity(vehicle)
				
				if speedoStatus == "none" then
					SendNUIMessage({
						act = "showSpeedo"
					})
					speedoStatus = "shown"
				...
				Citizen.SetTimeout(250, function()
					...
					if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() and seat == -1 then
						isDriver = true
						for id, data in pairs(vehicles) do
							if data[3] ~= 999999 then
								if data[3] == vehicle then
									vehicleId = id;
									break
								end
							end
				...
				while GetVehiclePedIsIn(PlayerPedId()) == vehicle do
					local hasVehicleDriver = GetPedInVehicleSeat(vehicle, -1) ~= 0
					local isInRightSeat = GetPedInVehicleSeat(vehicle, 0) == PlayerPedId()
					local isLocked = (GetVehicleDoorLockStatus(vehicle) >= 2)

					if speedoStatus == "shown" then
						SendNUIMessage({
							act = "setSpeedoData",
						...
						if isLocked then
							SendNUIMessage({act = "lockCar"});
						else
							SendNUIMessage({act = "unlockCar"});
						end
