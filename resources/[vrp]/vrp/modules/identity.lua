...
function generateRandomString()
  ...
  for i = 1, 16 do
      ...
      if i % 4 == 0 and i ~= 16 then
          randomString = randomString .. randomDigit .. " "
      else
          randomString = randomString .. randomDigit
      end
  end
...
function vRP.getUserIdentity(user_id, cbr)
  ...
  exports.oxmysql:execute("SELECT * FROM vrp_user_identities WHERE user_id = @user_id", {user_id = user_id}, function(rows)
    task({rows[1]})
  end)
end
...
exports.oxmysql:execute("SELECT user_id FROM vrp_user_identities WHERE registration = @registration", {registration = registration or ""}, function(rows, affected)
    if #rows > 0 then
      task({rows[1].user_id})
    else
      task()
...
function vRP.getUserByPhone(phone, cbr)
  ...
  exports.oxmysql:execute("SELECT user_id FROM vrp_user_identities WHERE phone = @phone", {phone = phone or ""}, function(rows, affected)
    if #rows > 0 then
      task({rows[1].user_id})
    else
      task()
    ...
    if identity == nil then
      vRP.generateRegistrationNumber(function(registration)
        vRP.generatePhoneNumber(function(phone)
         exports.oxmysql:execute("INSERT IGNORE INTO vrp_user_identities(user_id,registration,phone,firstname,name,age,card_number) VALUES(@user_id,@registration,@phone,@firstname,@name,@age,@card_number)", {
            user_id = user_id,
...
local function ch_identity(player,choice)
  ...
  if vRP.getInventoryItemAmount(user_id, "buletin") < 1 then 
  if user_id ~= nil then
        ...
        vRP.prompt(player,lang.cityhall.identity.prompt_name(),"",function(player,name)
          if string.len(name) >= 2 and string.len(name) < 50 then
            ...
            vRP.prompt(player,lang.cityhall.identity.prompt_age(),"",function(player,age)
              age = parseInt(age)
              if age >= 16 and age <= 80 then
                if vRP.tryPayment(user_id,cfg.new_identity_cost) then
                  vRP.generateRegistrationNumber(function(registration)
                    vRP.generatePhoneNumber(function(phone)
                vRP.giveInventoryItem(user_id,'buletin', 1, true) 
                     exports.oxmysql:execute("UPDATE vrp_user_identities SET firstname = @firstname, name = @name, age = @age, registration = @registration, phone = @phone WHERE user_id = @user_id", {
                        user_id = user_id,
                        firstname = firstname,
                        name = name,
                        age = age,
  ...
  vRPclient.notify(player,{' Error: Ai deja un buletin!'})
end
end
...
local function build_client_cityhall(source) -- build the city hall area/marker/blip
  ...
  if user_id ~= nil then
    ...
    vRP.setArea(source,"Creaza Buletin ",x,y,z,1,1.5,cityhall_enter,cityhall_leave)
  end
end
...
RegisterNetEvent("adm:showBuletin", function(player,choice)
	local user_id = vRP.getUserId(player)
	if user_id ~= nil then
	  vRP.getUserIdentity(user_id, function(identity)
		if identity then
		  ...
		  TriggerClientEvent('3dme:shareDisplay', -1, "~w~Isi verifica (~r~buletinul~w~)", player)
...
RegisterNetEvent("adm:showPermis", function(player,choice)
	local user_id = vRP.getUserId(player)
	if user_id ~= nil then
	  vRP.getUserIdentity(user_id, function(identity)
		if identity then
		  ...
		  local firstname = identity.firstname
		  local age = identity.age
		  TriggerClientEvent('3dme:shareDisplay', -1, "~w~Isi verifica  (~r~Permisul~w~) ~w~", player)
