    	...
    	if user_id ~= nil then
      		...
      		local choose = function(player, choice)
				...
				if vname then
					...
					local vehicle = vehicles[vname]
					if vehicle then
						local result = exports.oxmysql:executeSync("SELECT veh_type,vehicle_plate,upgrades FROM vrp_user_vehicles WHERE user_id = @user_id AND vehicle = @vehicle", {user_id = user_id, vehicle = vname})
						vRP.closeMenu(player)
			...
			local pvehicles = exports.oxmysql:executeSync("SELECT vehicle FROM vrp_user_vehicles WHERE user_id = @user_id", {user_id = user_id})
				...
				local choose = function(player, choice)
					vRP.prompt(player, "Esti sigur?", "[STERGE si scrie 'Da' pentru a vinde masina]", function(player,answer)
						...
						if(string.lower(answer) == "da")then
							...
							vRPclient.notify(player, {"Eroare: ~w~Trebuie sa scrii 'Da' pentru a vinde masina!"})
						end
					end)
				end
		...
		if user_id ~= nil then
			for k,v in pairs(garages) do
				...
				if group then
					...
					vRPclient.addMarker(source,{x,y,z-24,0.7,0.7,0.5,0,255,125,125,150})
	...
	vRP.prompt(player,"Placuta:","",function(player,vehPlate) 
		...
		if(string.len(vehicle_plate)<=8)then
			if(string.len(vehicle_plate)<=0)then
				...
				local kr = tonumber(vRP.getGiftpoints(user_id))
				if(vehicle_plate:match("%W")) then
					vRPclient.notify(player,{"Eroare: ~w~Placuta poate contine doar numere si litere!"})
				else
					if(kr >= 2) then
						exports.oxmysql:execute("UPDATE vrp_user_vehicles SET vehicle_plate = @vehicle_plate WHERE user_id = @user_id AND vehicle = @vehicle", {vehicle = vehicle, user_id = user_id, vehicle_plate = vehicle_plate}, function()end) 
						vRPclient.notify(player,{"Succes: ~w~Placuta de inmatriculare a fost schimbata in: ~o~"..vehicle_plate})
						vRPclient.despawnGarageVehicle(player,{vtype,15}) 
						vRP.setGiftpoints(user_id, kr-2)
						vRP.closeMenu({player})
					else
						vRPclient.notify(player,{"Eroare: ~w~Nu ai destule monede DonationCoins!"})
						vRP.closeMenu({player})
					end
				end
			end
		else
			vRPclient.notify(player,{"Eroare: ~w~Placuta de inmatriculare nu poate avea mai mult de 8 caractere!"})
		end
	end)
end, "Schimbare numar inmatriculare\nPret: 2 DonationCoins"}
			...
			if usrList ~= "" then
                ...
                if (tonumber(amount)) then
                  if tonumber(amount) > 0 then
					exports.oxmysql:execute('SELECT id FROM vrp_user_vehicles WHERE user_id = @user_id AND vehicle = @vehicle',{['user_id'] = user_id,['vehicle'] = name},function(pvehicle)
                      if #pvehicle > 0 then
                        vRPclient.notify(player,{"Eroare: ~w~Player-ul are deja masina"})
                      else
                          vRP.request(target,GetPlayerName(player).." wants to sell: " ..name.. " Price: $"..amount, 10, function(target,ok)
                            if ok then
                              ...
                              if (tonumber(money) >= tonumber(amount)) then
                                ...
                                vRPclient.notify(player,{"Succes: Ai vandut masina lui ".. GetPlayerName(target).." pentru $"..amount.."!"})
                                vRPclient.notify(target,{"Succes: "..GetPlayerName(player).." ti-a vandut masina pentru $"..amount.."!"})
                              else
                                vRPclient.notify(player,{"Eroare: ".. GetPlayerName(target).." nu are destui bani!"})
                                vRPclient.notify(target,{"Eroare: Nu ai destui bani"})
                              end
                            else
                              vRPclient.notify(player,{"Eroare: "..GetPlayerName(target).." a refuzat sa cumpere masina"})
                              vRPclient.notify(target,{"Succes: I-ai refuzat vanzarea lui "..GetPlayerName(player).." pentru masina"})
                            end
                          end)
                        vRP.closeMenu(player)
                      end
                    ...
                    vRPclient.notify(target,{"Eroare: Nu mai mere bug asta boss.","Error"})
                  end
								else
									vRPclient.notify(player,{"Eroare: Pretul masini trebuie sa fie un numar"})
								end
							end)
						else
						...
						vRPclient.notify(player,{"Eroare: Nu ai selectat un id"})
					end
				end)
			else
				vRPclient.notify(player,{"Eroare: Nu este niciun player pe langa tine"})
			end
		end)
	end
