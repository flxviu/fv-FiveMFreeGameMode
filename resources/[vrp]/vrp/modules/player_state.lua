...
AddEventHandler("vRP:playerSpawn", function(user_id, source, first_spawn)
	...
	if first_spawn then
		...
		if(vRP.tryFullPayment(user_id,50000))then
			vRPclient.notify(player, {"Succes: Ai platit 50.000 pentru taxa de spitalizare!"})
		end
...
RegisterCommand("cleanup", function(player, args)
	if player == 0 then
	  ...
	  TriggerClientEvent("chatMessage", -1, "^1Lucid: ^0Toate masinile neocupate vor fi sterse in ^1"..sec.." secunde^0")
