    ...
    if weapon then
      ...
      if weapon == "body_armor" then -- get player weapons to not rebuy the body
        ...
        if new_weight <= vRP.getInventoryMaxWeight(user_id) then
          ...
          vRPclient.notify(player,{"Eroare: ~w~Nu ai spatiu in buzunar"})
        end
      ...
      elseif weapon == "WEAPON_PISTOL" then
          vRP.prompt(player,lang.gunshop.prompt_ammo({choice}),"",function(player,amount)
            ...
            if amount >= 0 and amount ~= nil and amount ~= "" then
              if amount <= 250 then
                ...
                vRPclient.notify(player,{"Eroare: ~w~Poti cumpara doar pana la 250 de gloante!"})
              end
            else
              vRPclient.notify(player,{"Eroare: ~w~Valoare invalida!"})
            end
          end)
      else
        local hasOre = vRP.getUserHoursPlayed(user_id)
        if hasOre >= 5 or vRP.getUserFaction(user_id, "Politie") then
        vRP.prompt(player,lang.gunshop.prompt_ammo({choice}),"",function(player,amount)
          local amount = parseInt(amount)
          if amount >= 0 and amount ~= nil and amount ~= "" then
            if amount <= 250 then
        ...
        vRPclient.notify(player,{"Eroare: Nu ai 5 ore pentru a cumpara arma!"})
      end
      end
    end
  end
      ...
      if group and menu then
        ...
        local function gunshop_leave()
          ...
          if(gtype == gcfg.faction)then
            if gtype == "Politie" or gtype == "DIICOT" or gtype == "Smurd" then
              vRPclient.addBlip(source,{x,y,z,gcfg.blipid,gcfg.blipcolor,lang.gunshop.title({gtype})})
              vRPclient.addMarkerSign(source,{21,x,y,z-1.45,0.50,0.60,0.60,255,0,0,150,150,1,false,0})
              vRPclient.addMarkerNames(source,{x,y,z+0.350, "~w~ARMAMENT", 4, 1.2})          
            else
             vRPclient.addMarkerSign(source,{21,x,y,z-1.55,0.50,0.60,0.60,255,0,0,150,150,1,false,0})
             vRPclient.addMarkerNames(source,{x,y,z+0.350, "~w~ARSENAL", 4, 1.2})            
            end
          else
            vRPclient.addBlip(source,{x,y,z,gcfg.blipid,gcfg.blipcolor,lang.gunshop.title({gtype})})
            vRPclient.addMarkerSign(source,{21,x,y,z-1.55,0.50,0.60,0.60,255,0,0,150,150,1,false,0})
            vRPclient.addMarkerNames(source,{x,y,z+0.350, "~w~GUNSHOP", 4, 1.2})          
          end
