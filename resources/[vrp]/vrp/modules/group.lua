...
function vRP.removeUserAllGroups(user_id)
  local data = vRP.getUserDataTable(user_id)
  if data then
      if data.groups ~= nil then
          data.groups = {}
...
function vRP.removeUserGroup(user_id,group)
  ...
  if groupdef and groupdef._config and groupdef._config.onleave then
    if player ~= nil then
      groupdef._config.onleave(player) -- call leave callback
    end
  end
  ...
  if user_id ~= nil then
    for k,v in pairs(selectors) do
      ...
      if gcfg and menu then
        ...
        if gcfg == "Joburi Ilegale" then
          vRPclient.addMarker(source,{x,y,z-1,0.7,0.7,0.5,0,0,0,125})
          vRPclient.addMarkerNames(source,{x, y, z+0.520, "~n~Joburi Ilegale", 1, 1.0})
        else
          vRPclient.addBlip(source,{x,y,z,gcfg.blipid,gcfg.blipcolor,k})
          vRPclient.addMarker(source,{x,y,z-1,0.7,0.7,0.5,255,154,24,125})
          vRPclient.addMarkerNames(source,{x, y, z+0.520, "Joburi ~r~Legale ~w~2", 1, 1.0})
          vRPclient.addMarkerSign(source,{20,x,y,z-1.35,0.60,0.60,0.60,255,154,24,150,1,false,0})
        end
...
AddEventHandler("vRP:playerSpawn", function(user_id, source, first_spawn)
    ...
    build_client_selectors(source)
		...
		-- add fondator on user join 
    -- if user_id ~= nil then
      -- if user_id == 1 then
      --   vRP.setUserAdminLevel(user_id,11)
      -- end
		-- end
