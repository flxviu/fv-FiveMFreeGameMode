...
function vRP.getUserAddress(user_id, cbr)
  ...
  local rows = exports.ghmattimysql:executeSync("SELECT * FROM vrp_user_homes WHERE user_id = @user_id", {user_id = user_id})
  if #rows > 0 then
...
function vRP.setUserAddress(user_id,home,number)
  exports.ghmattimysql:execute("REPLACE INTO vrp_user_homes(user_id,home,number) VALUES(@user_id,@home,@number)", {user_id = user_id, home = home, number = number}, function()end)
end

-- remove user address
function vRP.removeUserAddress(user_id)
  exports.ghmattimysql:execute("DELETE FROM vrp_user_homes WHERE user_id = @user_id", {user_id = user_id}, function()end)
end

function vRP.sellHomeToPlayer(user_id, oldUser, home, number)
	exports.ghmattimysql:execute("UPDATE vrp_user_homes SET user_id = @user_id WHERE user_id = @oldUser AND home = @home AND number = @number", {user_id = user_id, oldUser = oldUser, home = home, number = number}, function()end)
end
...
function vRP.getUserByAddress(home,number,cbr)
  ...
  local rows = exports.ghmattimysql:executeSync("SELECT user_id FROM vrp_user_homes WHERE home = @home AND number = @number", {home = home, number = number})
  if #rows > 0 then
    task({rows[1].user_id})
  else
    task()
