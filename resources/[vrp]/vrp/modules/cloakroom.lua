  ...
  for k,v in pairs(cfg.cloakrooms) do
    ...
    if cloakroom and menu then
      ...
      local function cloakroom_enter(source,area)
        ...
        if user_id ~= nil and vRP.hasPermissions(user_id,gcfg.permissions or {}) and gcfg.faction == nil then
        ...
        if user_id ~= nil and gcfg.faction ~= nil and vRP.isUserInFaction(user_id,gcfg.faction) then
          if gcfg.not_uniform then -- not a uniform cloakroom
            -- notify player if wearing a uniform
            local data = vRP.getUserDataTable(user_id)
            if data.cloakroom_idle ~= nil then
      ...
      vRP.setArea(source,"Costumare - "..gtype.." | ID - "..k,x,y,z,1,1.5,cloakroom_enter,cloakroom_leave)
    end
  end
end
