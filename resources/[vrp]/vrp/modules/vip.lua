local titles = {"Vip Bronze", "Vip Silver", "Vip Gold", "Vip Diamond", "Vip Emerald"}
playerPerks = {}
...
function vRP.isUserVipEmerald(user_id)
    local vipRank = vRP.getUserVipRank(user_id)
    if (vipRank >= 5) then
        return true
    else
...
function vRP.setUserVip(user_id, vip, time)
    ...
    exports.oxmysql:execute('UPDATE vrp_users SET vipLvl = ' .. vip .. ', vipTime = ' .. vipHash .. ' WHERE id = ' .. user_id, {})
end
...
AddEventHandler('vRP:playerSpawn', function(user_id, player, first_spawn)
    local time = os.time()
    if first_spawn then
        exports.oxmysql:execute('SELECT vipLvl,vipTime FROM `vrp_users` WHERE id = ' .. user_id, {}, function(rows)
            if #rows > 0 then
                playerPerks[user_id] = {
                ...
                if playerPerks[user_id].vipLvl > 0 then
                    ...
                    if time > playerPerks[user_id].vipTime and playerPerks[user_id].vipTime > 0 then
                        if playerPerks[user_id].vipTime > 0 then
                            ...
                            exports.oxmysql:execute('UPDATE vrp_users SET vipLvl = 0, vipTime = 0 WHERE id = ' .. user_id, {})
...
vRP.registerMenuBuilder("jucator", function(add, data)
    ...
    if user_id ~= nil then
        ...
        if tmp then
            local time = os.time()
            vipstatus = "VIP Title: <font color='green'> " .. vRP.getUserVipTitle(user_id) .. "</font><br/><font color='white'>VIP Time:</font> <font color='red'>" .. formatOsTime(playerPerks[user_id].vipTime - time) .. "</font>"
            
            choices["VIP Status"] = {function(player, choice)
