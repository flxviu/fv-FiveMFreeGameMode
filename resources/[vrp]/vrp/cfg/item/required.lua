...
items["body_armor"] = {"🎽 Vesta","",function(args)
	local choices = {}
	local idname = args[1]

	choices["Echipeaza"] = {function(player,choice,mod)
		local user_id = vRP.getUserId(player)
		if user_id ~= nil then
			if vRP.tryGetInventoryItem(user_id, idname, 1, false) then
        vRPclient.notify(player, {"Echipezi 🎽 Vesta Anti-Glont Mare!"})
				vRPclient.setArmourica(player,{100,true,false})
				vRP.closeMenu(player)
			end
		end
	end}
...
items["buletin"] = {"Buletin","",function(args)
	local choices = {}
	local idname = args[1]
	...
	choices["Foloseste"] = {function(player,choice,mod)
	  local user_id = vRP.getUserId(player)
	  if user_id ~= nil then
	TriggerEvent('adm:showBuletin',player)
	  end
	end}
	return choices
  end,1.0}
  ...
  items["Permis"] = {"Permis","",function(args)
	local choices = {}
	local idname = args[1]
	...
	choices["Foloseste"] = {function(player,choice,mod)
	  local user_id = vRP.getUserId(player)
	  if user_id ~= nil then
	TriggerEvent('adm:showPermis',player)
	  end
	end}
	return choices
  end,1.0}
