vRPSfleeca = Tunnel.getInterface("vrp_fleeca","vrp_fleeca")
vRPCfleeca = {}
Tunnel.bindInterface("vrp_fleeca",vRPCfleeca)
Proxy.addInterface("vrp_fleeca",vRPCfleeca)
...
RegisterNetEvent('utk_fh:IsCop')
AddEventHandler('utk_fh:IsCop', function()
    IsPolice = true
end)
RegisterNetEvent('utk_fh:IsNOTCop')
AddEventHandler('utk_fh:IsNOTCop', function()
    IsPolice = false
end)
Freeze = {Alta = 0, Vinewood = 0, Highway = 0, Burton = 0, Desert = 0}
IsPolice = false
Check = {Alta = false, Vinewood = false, Highway = false, Burton = false, Desert = false}
SearchChecks = {Alta = false, Vinewood = false, Highway = false, Burton = false, Desert = false}
LootCheck = {
    Alta = {Stop = false, Loot1 = false, Loot2 = false, Loot3 = false},
    Vinewood = {Stop = false, Loot1 = false, Loot2 = false, Loot3 = false},
    Highway = {Stop = false, Loot1 = false, Loot2 = false, Loot3 = false},
    Burton = {Stop = false, Loot1 = false, Loot2 = false, Loot3 = false},
    Desert = {Stop = false, Loot1 = false, Loot2 = false, Loot3 = false}
}
Doors = {}
...
function ShowTimer() SetTextFont(4) SetTextProportional(0) SetTextScale(0.42, 0.42) SetTextDropShadow(0, 0, 0, 0,255) SetTextEdge(1, 0, 0, 0, 255) SetTextEntry("STRING") AddTextComponentString("~s~Timp Ramas ~r~"..UTK.timer.."~s~ secunde") DrawText(0.837, 0.910) end
                ...
                if dst < 40 then
                    if not LootCheck[name].Loot1 then
                        ...
                        if dst1 < 5 and not IsPolice then
                            DrawText3D(data.trolley1.x, data.trolley1.y, data.trolley1.z+1, "Apasa ~r~E~w~ pentru a lua ~b~banii", 0.30)
                    ...
                    if not LootCheck[name].Loot2 then
                        ...
                        if dst1 < 5 and not IsPolice then
                            DrawText3D(data.trolley2.x, data.trolley2.y, data.trolley2.z+1, "Apasa ~r~E~w~ pentru a lua ~b~banii", 0.30)
                    ...
                    if not LootCheck[name].Loot3 then
                        ...
                        if dst1 < 5 and not IsPolice then
                            DrawText3D(data.trolley3.x, data.trolley3.y, data.trolley3.z+1, "Apasa ~r~E~w~ pentru a lua ~b~banii", 0.30)
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
            ...
            if IsPolice and not dooruse then
                ...
                for k, v in pairs(Doors) do
                    for i = 1, 2, 1 do
                        ...
                        if dst <= 2.0 then
                            if v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "Apasa ~r~E~w~ pentru a deschide ~b~Usa", 0.30)
                            elseif not v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "Apasa ~r~E~w~ pentru a inchide ~b~Usa", 0.30)
                            end
...
AddEventHandler("utk_fh:toggleVault", function(key, state)
    ...
    if UTK.Banks[key].hash == nil then
        ...
        if not state then
            local obj = GetClosestObjectOfType(UTK.Banks.Highway.doors.startloc.x, UTK.Banks.Highway.doors.startloc.y, UTK.Banks.Highway.doors.startloc.z, 2.0, UTK.Banks.Highway.hash, false, false, false)
            local count = 0
            repeat
                local heading = GetEntityHeading(obj) - 0.10
                SetEntityHeading(obj, heading)
...
AddEventHandler("utk_fh:startheist", function(data, name)
    TriggerServerEvent("utk_fh:toggleDoor", name, true)
    disableinput = true
    robbing = true
    currentname = name
    currentcoords = vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z)
    ...
    exports.rprogress:Start("Folosesti cardul fals", 2000)
    Citizen.Wait(1500)
    ...
    disableinput = false
    Citizen.Wait(1000)
    exports.rprogress:Start("Hackuiesti", UTK.hacktime)
...
function SecondDoor(data, key)
    ...
    ClearPedTasksImmediately(ped)
    disableinput = false
    exports.rprogress:Start("Accesezi panoul de control", 2000)
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
...
function SpawnTrolleys(data, name)
    ...
    TriggerServerEvent("utk_fh:startLoot", data, name)
    done = false
end
...
Citizen.CreateThread(function()
    while true do
        if not done then
            ...
            if dst < 4 then
                DrawText3D(UTK.Banks[currentname].doors.secondloc.x, UTK.Banks[currentname].doors.secondloc.y, UTK.Banks[currentname].doors.secondloc.z, "Apasa ~r~E~w~ pentru a folosii cardul de ~b~securitate", 0.30)
                if dst < 0.75 and IsControlJustReleased(0 , 38) then
                    UTKFleeca.TriggerServerCallback("utk_fh:checkSecond", function(result)
                        if result then
                            done = true
...
Citizen.CreateThread(function()
    UTKFleeca.TriggerServerCallback("utk_fh:getBanks", function(bank, door)
        UTK.Banks = bank
        Doors = door
    end)
            ...
            for k, v in pairs(UTK.Banks) do
                if not v.onaction then
                    ...
                    if dst <= 2 and not Check[k] and not robbing then
                        DrawText3D(v.doors.startloc.x, v.doors.startloc.y, v.doors.startloc.z, "Apasa ~r~E ~w~pentru a incepe ~b~Heist-ul", 0.35)
