...
AddEventHandler('showBlips', function()
	...
	if showblip then
		showsprite = true
		vRP.notify({"Succes: ~w~Blips enabled"})
	else
		showsprite = false
		vRP.notify({"Eroare: ~w~Blips disabled"})
...
AddEventHandler('showSprites', function()
	...
	if showsprite then
		vRP.notify({"Succes: ~w~Sprites enabled"})
	else
		vRP.notify({"Succes: ~w~Sprites disabled"})
	end
end)
