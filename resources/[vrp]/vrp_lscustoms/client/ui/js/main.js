    ...
    window.addEventListener('message', (event) => {
        ...
        if (tempData.type === 'open') {
            ...
            if (tempData.what === 'menu') {
                ...
                fadeInColorPicker(tempData.title, tempData.price, tempData.defaultValue);
            }
...
function createMenu(menuId, data, title, defaultOption) {
    ...
    $('#options_container').stop(true, true).animate({height: '0px'}, 300, function() {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (data[i].price >= 0) {
                let tempPrice = Math.floor(data[i].price);

                let tempClass = '';
                if (tempPrice > currentCash) tempClass = 'cash_not';
                
                price = `<div class="options_container_option_price ${tempClass}">$${GetNumberWithCommas(tempPrice)}</div>`;
...
function fadeInColorPicker(title, price, defaultValue) {
    initColorPicker(title, price, defaultValue);
