...
local vRPserver = Tunnel.getInterface("tunning","tunning")
                ...
                for i = 1, #Config.Positions, 1 do
                    ...
                    if (tempDist <= tempActionDist) then
                        ...
                        if (not IsHudHidden()) then
                            DrawHelpText('Apasa '.. Config.Keys.action.name.. ' pentru a intra in ~r~Tuning', true)
                        end
...
function openUI()
    ...
    if (playerVeh ~= 0 and DoesEntityExist(playerVeh)) then
        ...
        for vtype, vehinfo in pairs(vehicles or {}) do
            if vehinfo[3] == playerVeh then
                customVehicleType = vtype
                break
            end
        end

        if customVehicleType ~= nil then

            if vtype == "car" then
                vRP.notify({"Eroare: ~w~Poti modifica doar masinile VIP sau cele detinute."})
                return
            end

            if GetVehicleEngineHealth(playerVeh) >= 700.0 then
                ...
                vRP.notify({"Eroare: ~w~Nu poti modifica un vehicul avariat!"})
            end
        else
            vRP.notify({"Eroare: ~w~Nu poti modifica un vehicul nedetinut!"})
        end
    end
end
                    ...
                    elseif (menuOption.price > 0) then
                        ...
                        if (currentMoney >= tempPrice) then
                            ...
                            if (tempPrice > 0) then
                                ...
                                TriggerServerEvent('lls-mechanic:removeMoney', tempPrice, vehicles[customVehicleType][2], customVehicleData)
                            end
                        end
                    end
                end
