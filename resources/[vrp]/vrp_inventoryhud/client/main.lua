...
RegisterNetEvent("vrp_inventory:forceCloseNUI")
AddEventHandler("vrp_inventory:forceCloseNUI",function()
...
RegisterNUICallback("UseItem", function(data, cb)
    ...
    TriggerServerEvent('toDiscord', '`Logs Inventar:` ' .. GetPlayerName(PlayerId()) .."[" .. GetPlayerServerId(PlayerId()) .."]" .. " **a folosit itemul: ** " .. data.item.name, 'https://discord.com/api/webhooks/1321589993197535333/OMSFZyVhmj1HYgCHqpCaWmF25G-epRN306VJS82i33mtfRogV07ASfDgjeMZQbOrTwkY')
    cb("ok")
end)

RegisterNUICallback("GiveItem", function(data, cb)
    INserver.requestItemGive({data.item.name, tonumber(data.number)})
    -- TriggerServerEvent('toDiscord', '`Logs Inventar:` ' .. GetPlayerName(PlayerId()) .."[" .. GetPlayerServerId(PlayerId()) .."]" .. " **a oferit: ** " .. data.item.name, 'https://discordapp.com/api/webhooks/1084296103060586557/q0YdEokpbwhRkg-1KHdMlvRU9GXw5H5tkVby4k6HCRetVgAQ1YdcxGtgkGthN2s0QkHp')
    cb("ok")
end)

RegisterNUICallback("GiveBani", function(data, cb)
    INserver.givebani({tonumber(data.number)})
    cb("ok")
end)
...
RegisterNetEvent("scoate:hotbar")
AddEventHandler("scoate:hotbar",function()
    TriggerServerEvent("scoate:hotbar",1)
    TriggerServerEvent("scoate:hotbar",2)
    TriggerServerEvent("scoate:hotbar",3)
    TriggerServerEvent("scoate:hotbar",4)
    TriggerServerEvent("scoate:hotbar",5)
end)
