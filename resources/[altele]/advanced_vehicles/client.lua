...
Citizen.CreateThread(function()
	TriggerServerEvent('advanceveh:set_job')
end)

RegisterNetEvent('advanceveh:set_job')
AddEventHandler('advanceveh:set_job', function(job)
	Job = job
end)
	...
	while true do
		...
		if IsThisModelACar(GetEntityModel(veh)) then
			if seat == ped and vehicleData == nil then
				...
				timer = 1000
				vehicleData = {}
				vehicleData.plate = GetPlate(GetVehicleNumberPlateText(veh))
	...
	for k,v in pairs(Config.repair[repair].repair) do
		...
		elseif v == "body" then
			...
			if vehiclesHandlingsOriginal[vehicleDataAction.name] and vehiclesHandlingsOriginal[vehicleDataAction.name][v] then
				...
				vehiclesHandlingsDamaged[vehicleDataAction.name][v] = nil
