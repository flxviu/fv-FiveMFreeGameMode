...
AddEventHandler('InteractSound_CL:PlayWithinDistance', function(playerNetId, maxDistance, soundFile, soundVolume)
    if hasSpawned then
        local ply = GetPlayerFromServerId(playerNetId)
        if NetworkIsPlayerActive(ply) then
            local lCoords = GetEntityCoords(GetPlayerPed(-1))
            local eCoords = GetEntityCoords(GetPlayerPed(ply))
            local distIs  = Vdist(lCoords.x, lCoords.y, lCoords.z, eCoords.x, eCoords.y, eCoords.z)
            if(distIs <= maxDistance) then
                if not soundsPlaying[soundFile] then
                    soundsPlaying[soundFile] = true
                    if distIs > 0 then
                        soundVolume = soundVolume - (distIs/(maxDistance*2))
...
RegisterNetEvent("InteractSound_CL:StopSound")
AddEventHandler("InteractSound_CL:StopSound", function()
